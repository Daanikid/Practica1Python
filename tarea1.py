...
def select_04():
    ...
    conn = conectar()
    cursor = conn.cursor()
    cursor.execute("SELECT e.nombre, c.nombre, i.nota  FROM inscripciones i JOIN estudiantes e ON i.estudiante_id = e.id JOIN cursos c ON i.curso_id = c.id")  # <-- completa aquí
    for r in cursor.fetchall():
        print(dict(r))
    conn.close()
...
def insert_04():
    ...
    conn = conectar()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO estudiantes(nombre, apellido, email, fecha_nacimiento) VALUES (?, ?, ?, ?)", ("Valentina", "Ruiz", "valentina.ruiz@email.com", "2003-05-20"))
    cursor.execute("INSERT INTO estudiantes(nombre, apellido, email, fecha_nacimiento) VALUES (?, ?, ?, ?)", ("Mateo", "Torres", "mateo.torres@email.com", "2004-11-15"))
    conn.commit()
